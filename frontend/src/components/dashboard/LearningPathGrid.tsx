    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paths.map((path, index) => (
                <Link
                    key={path.id}
                    href={path.slug ? `/courses/${path.slug}` : '#'}
                    className="block"
                >
                    <motion.div
                        ...
                        className="group bg-white rounded-3xl h-full overflow-hidden border border-gray-100 hover:border-blue-200 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 cursor-pointer"
                    >
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex items-center gap-1.5 text-xs text-gray-400">
                                    <BarChart size={14} />
                                    <span>{path.id === 1 ? '15 Chapters' : '8 Lessons'}</span>
                                </div>
                            </div>
